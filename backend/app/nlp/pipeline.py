    ...
    def get_statistics(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "entity_types": self._count_by_type(
                self.entities, lambda e: e.entity_type.value
            ),
            "relation_count": len(self.relations),
            "relation_types": self._count_by_type(
                self.relations, lambda r: r.predicate.value
            ),
    ...
    def get_knowledge_graph_nodes(self) -> List[Dict[str, Any]]:
        ...
        for entity in self.entities:
            ...
            if key not in seen:
                seen.add(key)
                nodes.append({
                    "id": hashlib.md5(f"{entity.text}_{entity.entity_type.value}".encode()).hexdigest()[:12],
                    "label": entity.text,
...
class NLPPipeline:
    ...
    def __init__(
        ...
        self._ner: Optional[TransformerNER] = None
        self._relation_extractor: Optional[RelationExtractor] = None
        self._assertion_detector: Optional[AssertionDetector] = None
        self._context_tagger: Optional[ContextTagger] = None
        self._domain_encoder: Optional[DomainRelationEncoder] = None

        self._ner_model = ner_model
        self._device = device

        logger.info(f"NLPPipeline initialized with components: "
    ...
    def context_tagger(self) -> ContextTagger:
        ...
        if self._context_tagger is None:
            self._context_tagger = ContextTagger(
                confidence_threshold=self.confidence_threshold
            )
    ...
    def analyze(
        ...
        if self.enable_domain_encoding and analysis.relations:
            try:
                ...
                adc_relations = self.domain_encoder.encode_adc_relations(
                    text, analysis.entities
                )
        ...
        if self.enable_assertions:
            try:
                ...
                analysis.assertions = [
                    self.assertion_detector.detect_assertion(sent)
            ...
            with ThreadPoolExecutor(max_workers=self.max_workers) as executor:
                ...
                for doc in documents:
                    ...
                    future = executor.submit(
                        self.analyze, text, doc_id, metadata
                    )
        ...
        for edge in edges:
            ...
            if key not in merged:
                ...
                merged[key]["evidence_weight"] = max(
                    merged[key]["evidence_weight"],
                    edge["evidence_weight"]
                )
                merged[key]["evidence_count"] += 1
                merged[key]["source_documents"].extend(
                    edge.get("source_documents", [])
                )
    ...
    def get_pipeline_info(self) -> Dict[str, Any]:
        ...
        return {
            "components": {
                "ner": {
                    "enabled": self.enable_ner,
                    "model": self._ner_model
                },
                "relation_extraction": {
                    "enabled": self.enable_relations
                },
                "assertion_detection": {
                    "enabled": self.enable_assertions
                },
                "context_tagging": {
                    "enabled": self.enable_context
                },
                "domain_encoding": {
                    "enabled": self.enable_domain_encoding
                }
            },
            "settings": {
                "confidence_threshold": self.confidence_threshold,
