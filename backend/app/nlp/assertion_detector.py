...
class AssertionDetector:
    ...
    def __init__(
        ...
        detect_negation_scope: bool = True
    ):
        """
        ...
        self._positive_patterns = [re.compile(p, re.IGNORECASE) for p in self.DEFINITE_POSITIVE_CUES]
        self._conditional_patterns = [re.compile(p, re.IGNORECASE) for p in self.CONDITIONAL_CUES]
        self._contraindication_patterns = [re.compile(p, re.IGNORECASE) for p in self.CONTRAINDICATION_CUES]
        ...
        for strength, patterns in self.EVIDENCE_INDICATORS.items():
            self._evidence_patterns[strength] = [
...
def analyze_claim(claim_text: str, context: Optional[str] = None) -> Dict[str, Any]:
