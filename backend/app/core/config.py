...
class Settings(BaseSettings):
    ...
    # Phase 2 of the discovery_orchestrator → grounded-agent-layer
    # migration (task #74). Stages whose number is in this list route
    # through `app/agents/agent_layer_shim.py` instead of
    # MultiModelLLM.generate(). Operator can override per-environment.
    # Phasing recap:
    #   Phase 1: [12]            FINALIZE only — synthesis is the
    #                              least-risky stage to run through
    #                              the new path (already grounded by
    #                              stages 1-11). Done.
    #   Phase 2: [9, 10, 11, 12] post-grounding stages — current
    #                              default. The grounding gate has
    #                              already filtered upstream stage
    #                              output by the time these run, so
    #                              the agent-layer's tool-calling
    #                              ability adds value without risking
    #                              the citation chain.
    #   Phase 3: [1..12]         full migration once Phase 2 has run
    #                              in production for a full cohort of
    #                              real discoveries.
    USE_AGENT_LAYER_FOR_STAGES: list[int] = [9, 10, 11, 12]
