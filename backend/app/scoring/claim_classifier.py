...
class ClaimClassifier:
    ...
    CLAIM_TYPE_PATTERNS = {
        ...
        ClaimType.THERAPEUTIC: [
            ...
            r"\btherapeutic\b",
            r"\befficac(?:y|ious)\b",
            r"\beffective\s+(?:in|for|against)\b",
            r"\bbenefit\b",
        ],
        ClaimType.DIAGNOSTIC: [
            r"\bdiagnos(?:es?|tic|is)\b",
            r"\bdetects?\b",
            r"\bidentif(?:y|ies)\b",
            r"\bmarker\s+for\b",
        ],
    ...
    def _determine_direction(
        self, negation_cues: list[str], hedging_cues: list[str], text: str
    ) -> ClaimDirection:
    ...
    def _extract_quantitative(self, text: str) -> dict[str, Any]:
        ...
        # Confidence interval
        ci_match = re.search(
            r"(?:95%?\s*)?CI\s*[:\[]\s*(\d+\.?\d*)\s*[-,]\s*(\d+\.?\d*)", text, re.IGNORECASE
        )
        if ci_match:
            data["confidence_interval"] = [float(ci_match.group(1)), float(ci_match.group(2))]
    ...
    def analyze_claims(self, classifications: list[ClaimClassification]) -> dict[str, Any]:
