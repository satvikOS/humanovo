...
def gene_set_enrichment():
    ...
    try:
        ...
        for pid, pdata in pathways.items():
            ...
            abs_nes = abs(nes)
            p_val = 0.001 if abs_nes > 3.0 else (0.01 if abs_nes > 2.0 else (0.05 if abs_nes > 1.5 else (0.1 if abs_nes > 1.0 else min(1.0, 0.5 + (1 - abs_nes) * 0.5))))
        ...
        for i, r in enumerate(sorted(results, key=lambda x: x["p_value"])):
            r["fdr"] = round(min(1.0, r["p_value"] * n_tests / (i + 1)), 4)
...
def discover_biomarkers():
    ...
    try:
        ...
        for entry in expression_data:
            ...
            # Welch's t-test (direction matches fold change: group2 - group1)
