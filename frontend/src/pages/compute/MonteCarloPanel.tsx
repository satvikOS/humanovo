              ...
              <div style={{ flex: 1, display: 'flex', flexDirection: 'column', minHeight: 0 }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 2, marginBottom: 6 }}>
                  ...
                  <span style={{ fontSize: 10, color: 'var(--color-text-muted)', marginLeft: 'auto' }}>
                    {results.values.length.toLocaleString()} iterations
                  </span>
                  {/* Raw-data exports only. Image export (PNG / SVG /
                      PDF / copy-figure) lives in each figure's own
                      PublicationFigure toolbar — duplicating it here was
                      redundant chrome. */}
                  <div style={{ display: 'flex', gap: 4, marginLeft: 8 }}>
                    <button style={styles.exportBtn} onClick={handleExportCSV} title="Download raw values as CSV"><FiDownload style={{ fontSize: 9 }} /> CSV</button>
                    <button style={styles.exportBtn} onClick={handleExportJSON} title="Download results + stats as JSON"><FiDownload style={{ fontSize: 9 }} /> JSON</button>
