...
export default function PresetRunner() {
  ...
  const renderChart = (res: ComputeResult) => {
    ...
    const axisStyle = { fontSize: 9, fill: 'var(--color-text-muted)' }
    const gridStyle = { strokeDasharray: '3 3', stroke: 'var(--color-text-muted)', opacity: 0.1 }
        ...
        return (
          <ResponsiveContainer width="100%" height={220}>
            <ScatterChart margin={margin}>
              ...
              <XAxis dataKey="x" type="number" tick={axisStyle} label={res.xLabel ? { value: res.xLabel, position: 'bottom', style: { ...axisStyle, fontSize: 8 } } : undefined} />
              <YAxis dataKey="y" type="number" tick={axisStyle} label={res.yLabel ? { value: res.yLabel, angle: -90, position: 'left', style: { ...axisStyle, fontSize: 8 } } : undefined} />
              <Tooltip content={<ChartTip />} />
              <Scatter data={res.chartData} fill="var(--color-accent-blue)" r={2.5} fillOpacity={0.7} />
            </ScatterChart>
          </ResponsiveContainer>
        )
      case 'bar':
        return (
          <ResponsiveContainer width="100%" height={220}>
            <BarChart data={res.chartData} margin={margin}>
              ...
              <Tooltip content={<ChartTip />} />
              <Bar dataKey="y" fill="var(--color-accent-blue)" radius={[2, 2, 0, 0]} />
            </BarChart>
          </ResponsiveContainer>
        )
      case 'area':
        return (
          <ResponsiveContainer width="100%" height={220}>
            <AreaChart data={res.chartData} margin={margin}>
              ...
              <YAxis tick={axisStyle} />
              <Tooltip content={<ChartTip />} />
              <Area type="monotone" dataKey="y" stroke="var(--color-accent-blue)" fill="var(--color-accent-blue)" fillOpacity={0.2} />
            </AreaChart>
          </ResponsiveContainer>
        )
      case 'multi-line':
        return (
          <ResponsiveContainer width="100%" height={220}>
            <LineChart data={res.chartData} margin={margin}>
              ...
              <YAxis tick={axisStyle} />
              <Tooltip content={<ChartTip />} />
              <Line type="monotone" dataKey="y" stroke="var(--color-accent-blue)" dot={false} strokeWidth={1.5} name={res.seriesLabels?.[0] || 'y'} />
              {res.chartData.some(d => d.y2 !== undefined) && (
                <Line type="monotone" dataKey="y2" stroke="#22c55e" dot={false} strokeWidth={1.5} name={res.seriesLabels?.[1] || 'y2'} />
              )}
              {res.chartData.some(d => d.y3 !== undefined) && (
                <Line type="monotone" dataKey="y3" stroke="#a855f7" dot={false} strokeWidth={1.5} name={res.seriesLabels?.[2] || 'y3'} />
              )}
            </LineChart>
          </ResponsiveContainer>
        )
        ...
        return (
          <ResponsiveContainer width="100%" height={220}>
            <LineChart data={res.chartData} margin={margin}>
              <CartesianGrid {...gridStyle} />
              <XAxis dataKey="x" type="number" tick={axisStyle} label={res.xLabel ? { value: res.xLabel, position: 'bottom', style: { ...axisStyle, fontSize: 8 } } : undefined} />
              <YAxis tick={axisStyle} label={res.yLabel ? { value: res.yLabel, angle: -90, position: 'left', style: { ...axisStyle, fontSize: 8 } } : undefined} />
              <Tooltip content={<ChartTip />} />
              <Line type="monotone" dataKey="y" stroke="var(--color-accent-blue)" dot={false} strokeWidth={1.5} />
            </LineChart>
          </ResponsiveContainer>
        )
    }
  ...
  return (
    <div className="flex h-full overflow-hidden">
      ...
      <div className="w-48 flex-shrink-0 border-r overflow-y-auto p-2 space-y-1" style={{ borderColor: 'var(--glass-border)' }}>
        <button
          onClick={() => { setSelectedToolbox(null); setSelectedPreset(null) }}
          className={clsx('w-full flex items-center gap-2 px-3 py-2 rounded-md text-xs font-medium transition-colors', !selectedToolbox && 'ring-1')}
          style={{
            background: !selectedToolbox ? 'var(--color-accent-blue)' : 'transparent',
            color: !selectedToolbox ? '#fff' : 'var(--color-text)',
            outlineColor: !selectedToolbox ? 'var(--color-accent-blue)' : undefined,
          }}
        >
          <FiGridIcon className="text-sm" />
          <span>All Toolboxes</span>
          <span className="ml-auto opacity-60">{ALL_PRESETS.length}</span>
        </button>
        ...
        {categories.map(cat => {
          ...
          return (
            <button
              ...
              className={clsx('w-full flex items-center gap-2 px-3 py-2 rounded-md text-xs transition-colors', active && 'ring-1')}
              style={{
                background: active ? cat.color + '22' : 'transparent',
                color: active ? cat.color : 'var(--color-text)',
                outlineColor: active ? cat.color : undefined,
              }}
            >
              <Icon className="text-sm" style={{ color: cat.color }} />
              <span className="truncate">{cat.name}</span>
              <span className="ml-auto opacity-60">{cat.presetCount}</span>
            </button>
          )
        })}
      </div>
        ...
        {!selectedPreset ? (
              ...
              {filteredPresets.map(preset => {
                ...
                return (
                  <button
                    ...
                    className="group text-left p-3 rounded-lg border transition-all hover:scale-[1.01]"
                    style={{ background: 'var(--glass-bg)', borderColor: 'var(--glass-border)' }}
                  >
                    <div className="flex items-start justify-between">
                      <div className="flex-1 min-w-0">
                        <h3 className="text-xs font-semibold truncate" style={{ color: 'var(--color-text)' }}>{preset.name}</h3>
                        <span className="text-[10px] font-mono" style={{ color: cat?.color || 'var(--color-text-muted)' }}>{preset.matlabFn}</span>
                      </div>
                    ...
                    <p className="text-[10px] mt-1 line-clamp-2" style={{ color: 'var(--color-text-muted)' }}>{preset.description}</p>
                    <div className="flex items-center gap-1.5 mt-2">
                      <span className="px-1.5 py-0.5 rounded text-[9px] font-medium" style={{ background: (cat?.color || '#888') + '22', color: cat?.color || '#888' }}>
                        {cat?.name || preset.toolbox}
                      </span>
                      {preset.workflowStage && (
                        <span className="px-1.5 py-0.5 rounded text-[9px]" style={{ background: 'var(--glass-border)', color: 'var(--color-text-muted)' }}>
                          {preset.workflowStage}
                        </span>
                      )}
                    </div>
          ...
          <>
            ...
            <div className="mb-4">
              ...
              <p className="text-xs mt-0.5" style={{ color: 'var(--color-text-muted)' }}>{selectedPreset.description}</p>
              <span className="inline-block text-[10px] font-mono mt-1 px-1.5 py-0.5 rounded" style={{ background: 'var(--glass-border)', color: 'var(--color-text-muted)' }}>
            ...
            {showCode && (
              <pre className="text-[10px] p-3 rounded-md border mb-4 overflow-x-auto font-mono whitespace-pre-wrap" style={{ background: 'var(--glass-bg)', borderColor: 'var(--glass-border)', color: 'var(--color-text-muted)' }}>
              ...
              {selectedPreset.params.map(p => (
                <div key={p.key}>
                  <label className="text-[10px] font-medium block mb-0.5" style={{ color: 'var(--color-text-muted)' }}>
                    {p.label}
                    {p.description && <span className="font-normal ml-1 opacity-60">— {p.description}</span>}
                  </label>
                  {p.type === 'textarea' ? (
                    <textarea
                      value={params[p.key] || ''}
            ...
            <button
              ...
              style={{
                background: running ? 'var(--glass-border)' : 'var(--color-accent-blue)',
                color: '#fff',
              }}
            >
            ...
            {error && (
              <div className="flex items-center gap-2 mt-3 p-2 rounded-md text-xs" style={{ background: '#ef444422', color: '#ef4444' }}>
                <FiAlertCircle /> {error}
              </div>
            )}
          </>
      ...
      <div className="w-[400px] flex-shrink-0 border-l overflow-y-auto p-4" style={{ borderColor: 'var(--glass-border)' }}>
        {result ? (
          <>
            <div className="flex items-center justify-between mb-3">
              ...
              <div className="flex gap-1">
                <button onClick={copyStats} className="p-1 rounded hover:bg-white/5" title="Copy"><FiCopy className="text-xs" style={{ color: 'var(--color-text-muted)' }} /></button>
                <button onClick={exportJSON} className="p-1 rounded hover:bg-white/5" title="JSON"><FiDownload className="text-xs" style={{ color: 'var(--color-text-muted)' }} /></button>
                <button onClick={exportCSV} className="p-1 rounded hover:bg-white/5" title="CSV"><FiDownload className="text-xs" style={{ color: 'var(--color-accent-blue)' }} /></button>
              </div>
            </div>

            {result.warnings?.map((w, i) => (
              <div key={i} className="flex items-center gap-1.5 mb-2 px-2 py-1 rounded text-[10px]" style={{ background: '#f59e0b22', color: '#f59e0b' }}>
            ...
            {result.chartData?.length ? (
              <div className="mb-4 rounded-lg border p-2" style={{ borderColor: 'var(--glass-border)' }}>
                {result.chartTitle && (
                  <div className="text-[10px] font-medium text-center mb-1" style={{ color: 'var(--color-text-muted)' }}>{result.chartTitle}</div>
                )}
                {renderChart(result)}
              </div>
            ) : null}
            ...
            {result.statistics?.length ? (
              <div className="rounded-lg border overflow-hidden" style={{ borderColor: 'var(--glass-border)' }}>
          ...
          <div className="flex flex-col items-center justify-center h-full opacity-30">
            <FiGridIcon className="text-3xl mb-3" />
            <p className="text-xs">Select a preset and run it</p>
            <p className="text-[10px] mt-1">Results appear here</p>
          </div>
        )}
      </div>
    </div>
