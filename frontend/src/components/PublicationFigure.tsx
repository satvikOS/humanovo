...
export default function PublicationFigure({
  ...
  const exportPdf = useCallback(async () => {
    ...
    try {
      ...
      const margin = 36
      // IEEE is a serif theme — render the PDF text block in Times so
      // the caption/title typography matches the figure's theme. The
      // sans themes (Nature/Science/paper/screen) stay on Helvetica.
      const pdfFont = theme === 'ieee' ? 'times' : 'helvetica'
  ...
  }, [exportStem, title, subtitle, caption, source, theme])
