    ...
    <ResponsiveContainer width="100%" height={height}>
      <ScatterChart data={data} margin={{ top: 8, right: 16, bottom: 24, left: 8 }}>
        ...
        {showSmoother && smoother.length > 0 && (
          ...
          // Recharts' Scatter `data` prop is typed loosely; cast through
          // `unknown` to keep `any` out of the codebase.
          <Scatter name="Trend" data={smoother as unknown as Array<Record<string, number>>} fill={colors[3] || colors[1]} line={{ stroke: colors[3] || colors[1], strokeWidth: 2 }} shape="circle" />
        )}
      </ScatterChart>
    </ResponsiveContainer>
  )
...
// computeRegressionDiagnostics and the RegressionDiagnostics type moved
// to utils/regressionDiagnostics.ts. Components/*.tsx must export only
// React components for Fast Refresh to work cleanly.
